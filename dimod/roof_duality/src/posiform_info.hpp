...
class PosiformInfo {
	public: 
		...
		PosiformInfo(const BQM& bqm) {
			...
			// Apart from finding the maximum absolute value in the 
			// qubo, we must consider the sum of the absolute values of
			// linear values found by using non integer bqm-biases for calculating
			// the conversion ratio. As that value converted to integer is the 
			// upper bound for max flow. 
			// This is because the linear values correspond to the capacities 
			// of the edges in the implication network connecting to source/sink.
			// Thus we calculate the linear values of posiform in double format.
			// Note in the posiform all linear values are positive, here we keep
			// the sign to indicate whether X_0 is connected to X_i or X_i'.
			...
			// Consider the upper limit of max-flow in implication graph
			// for calculating ratio conversion ratio.
			...
			_bias_conversion_ratio = static_cast<double>(std::numeric_limits<long long int>::max()) / _max_absolute_value;

			// TODO: We should be removing this if we consider the sum of the linear biases
			// as mentioned above, but we may divide by 2 to be safe. We must not clamp the
			// ratio to 1 as it is done now.
			_bias_conversion_ratio /= static_cast<double>(1LL << 10);
			if (_bias_conversion_ratio < 1)
				_bias_conversion_ratio = 1;

			_bias_conversion_ratio_limit = static_cast<double>(std::numeric_limits<long long int>::max()) / std::fabs(_posiform_linear_sum_non_integer);
		...
		void print() {
			...
			std::cout <<"Numeric Limit of long long int " << std::numeric_limits<long long int>::max() <<std::endl;
			std::cout <<"Linear Sum Unconverted : " << _posiform_linear_sum_non_integer << std::endl;
			std::cout <<"Linear Sum Converted : " << convertToLL(_posiform_linear_sum_non_integer) <<std::endl;
			std::cout <<"Linear Sum After Summing : " << _posiform_linear_sum_integer <<std::endl;
			std::cout <<"Chosen Ratio : " << _bias_conversion_ratio <<std::endl;
			std::cout <<"Ratio Considering Linear Sum : " << _bias_conversion_ratio_limit << std::endl;
			std::cout << std::endl;

			// Printing out in convluted manner, to verify the mappings.
			std::cout << "Used Variables (bqm --> posiform) : " << _posiform_to_bqm_variable_map.size() <<  std::endl;
			for(int i = 0; i < _posiform_to_bqm_variable_map.size(); i++) {
				std::cout << _posiform_to_bqm_variable_map[i] << " --> " << _bqm_to_posiform_variable_map[_posiform_to_bqm_variable_map[i]] << std::endl;
			}
			...
			for(int i =0; i < _num_bqm_variables; i++) {
				if(_linear_integer_biases[i]) {
					std::cout << _bqm_to_posiform_variable_map[i] <<", " <<  i <<", " << _linear_integer_biases[i] << std::endl;
				}
		        }
			...
			for(int i =0; i < _num_bqm_variables; i++) {
				...
				auto itEnd = _quadratic_iterators[i].second;
				for(; it != itEnd; it++){
					std::cout << _bqm_to_posiform_variable_map[i] <<" " << _bqm_to_posiform_variable_map[it->first] << ", "; 
		...
		inline long long int getLinear(int i) { return _linear_integer_biases[_posiform_to_bqm_variable_map[i]]; }
		inline int getNumQuadratic(int i) { return _num_quadratic_integer_biases[_posiform_to_bqm_variable_map[i]]; }

		// These functions do not expose the bqm, but the posiform, 
		// but for iterating ver the quadratic biases, we need the
		// convertToLL and gttMappedVariable functions.
		...
		inline int getMappedVariable(int i) { return _bqm_to_posiform_variable_map[i]; }
	...
	private:
		...
		std::vector<std::pair<quadratic_iterator_type,quadratic_iterator_type>> _quadratic_iterators;
};
